...
class UtilsSsusi(object):
    ...
    def overlay_sat_data(self, filteredDict, ax, crs=cartopy.crs.PlateCarree(),\
                         satList=["F18", "F17", "F16"], plotType='d135',\
                         overlayTime=True, overlayTimeInterval=5, timeLinestyle=':',\
                         timeColor="black", timeFontSize=6., zorder=5., timeZorder=7.,\
                         plotCBar=False, cbar_shrink=0.8, autoScale=True,
                         vmin=0., vmax=1000.,\
        ...
        for key in filteredDict.keys():
            ...
            xVecs, yVecs = ssusiLons, ssusiLats#mapHandle(ssusiLons, ssusiLats, coords=coords)
            # ssusiPlot = mapHandle.scatter(xVecs, yVecs, c=ssusiDisk, s=10.,\
            #            cmap=ssusiCmap, alpha=0.7, zorder=zorder, \
            #                      edgecolor='none', marker="s",\
            #                       vmin=vmin, vmax=vmax)
            ssusiPlot = ax.pcolormesh(xVecs, yVecs,\
                            ssusiDisk, zorder=zorder,
                            vmin=0, vmax=vmax,
                            transform=crs, alpha=alpha, cmap=ssusiCmap, shading='flat')
            ...
            if overlayTime:
                ...
                for dd in range( len(allDayDatesList) ):
                    for pixel in range(timessusiLons.shape[1]):
                        ...
                        xTVecs, yTVecs = timePlotLonArr, timePlotLatArr
#                         mapHandle(timePlotLonArr,\
#                                          timePlotLatArr, coords=coords)
                        ...
                        if pixel == timessusiLons.shape[1]-1:
                            ...
                            timeXVecs, timeYVecs = timePlotLonArr,timePlotLatArr
#                             mapHandle(timePlotLonArr,\
#                                  timePlotLatArr, coords=coords)
                            t_plot = ax.text(timeXVecs, timeYVecs, timeStr,\
                                fontsize=timeFontSize,
                                ha='left',va='center',color='k',\
                                 clip_on=True, zorder=timeZorder, transform=crs)
                            t_plot.clipbox = ax.bbox
                    
                    # this is for plotting a straight line!!!
                    geod_proj = cartopy.crs.Geodetic()
                    nps_proj = cartopy.crs.NorthPolarStereo()
                    
                    xTVecFirst, yTVecFirst = nps_proj.transform_point(
                        xTVecFirst, yTVecFirst, geod_proj
                    )
                    xTVecLast, yTVecLast = nps_proj.transform_point(
                        xTVecLast, yTVecLast, geod_proj
                    )
                
                    ax.plot([xTVecFirst, xTVecLast], [yTVecFirst, yTVecLast],\
                             timeColor, transform=nps_proj,\
                            zorder=timeZorder, linestyle=timeLinestyle)
            # plot colorbar
            if plotCBar:
                cbar = ax.colorbar(ssusiPlot, orientation='vertical', shrink=cbar_shrink)
    ...
    def convert_aacgm_geo(self, row, a2g=False):
        ...
        for i in range( row["shapeArr"] ):
            indStr = str(i+1)
            if a2g:
                aacgmv2.get_aacgm_coord(_gc[1], _gc[0], out_height, self.plot_date)
                glat, glon = aacgmv2.convert_latlon_arr(\
                                row["mlat." + indStr], row["mlon." + indStr],\
                               300, row["date"], method_code="G2A")
                row["glat." + indStr] = numpy.round( glat, 2)
                row["glon." + indStr] = numpy.round( glon, 2)
            else:
                mlat, mlon, aacgm_alt = aacgmv2.get_aacgm_coord(\
                                    row["glat." + indStr], row["glon." + indStr],\
                                   300, row["date"])
...
#                 asds
                ...
                row["mlt." + indStr] = numpy.round( mlt[0], 2)
        return row
